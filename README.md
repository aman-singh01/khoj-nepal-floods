...
| Official-services directory | `/official` page + a per-record "Also contact an official service" panel filtered by nationality, driven by [`src/config/official-sources.ts`](src/config/official-sources.ts) — the operator fills in verified portal URLs and MEA / MoFA helplines (phone numbers ship empty by design). |
