...
| Interop | `GET /api/pfif` exports Khoj's own published records as [PFIF 1.4](https://zesty.ca/pfif/1.4/) (`?since=` for deltas; imported records are excluded so data can't loop). `POST /api/pfif/import` (token-gated) ingests PFIF / CSV / JSON. Imported records carry their source's verification status (a feed that flags rows unverified is shown as **unverified**, with a callout), are attributed "via …", have phone/email/passport/DOB scrubbed from free text, and upsert idempotently. Scheduled via `vercel.json` cron `/api/feeds/refresh`, the GitHub Action, or `npm run feeds:pull` / `data:watch`. **Enabled now:** `flood.sodhera.com` (a peer community registry — ~150 unverified missing/found records). See [docs/INTEGRATIONS.md](docs/INTEGRATIONS.md). |
...
| Event news stream | `/updates` + a home "Latest updates" card aggregate coverage of the emergency from Google News and UN News RSS (ReliefWeb once you register an `appname`), deduped by URL, each tagged `official` / `humanitarian` / `news` and linked to its origin. Refreshes on page view (12-min TTL) **and on a schedule** — `vercel.json` (Vercel Cron) and `.github/workflows/pull-updates.yml` (GitHub Actions) both pull every 6 h; or run `npm run updates:pull` from any cron. Moderators pin or hide items on `/moderation`. Config: [`src/config/update-feeds.ts`](src/config/update-feeds.ts). |
